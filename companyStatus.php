...
$pdfFilePath = __DIR__ . "/registration_{$application_id}.pdf";
...
try {
    ...
    $mail->AddEmbeddedImage('logo.jpg', 'logoimg'); // local path, cid 'logoimg'


// ids
foreach ($shareholders as $s) {
    // ID Front
    $idFrontPath = __DIR__ . '/' . $s['id_front'];
    if (file_exists($idFrontPath)) {
        $mail->addAttachment($idFrontPath, $s['forenames'] . '_ID_Front' . pathinfo($idFrontPath, PATHINFO_EXTENSION));
    }

    // ID Back
    $idBackPath = __DIR__ . '/' . $s['id_back'];
